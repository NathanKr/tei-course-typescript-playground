...
export function onExpenseAdded() {
  ...
  getExpensesElem().innerHTML += newItemElem;
}

function getExpensesElem(): HTMLDivElement {
...
export function updateAllExpencesPercentage() {
  // ---- relevant only on income add or delete
  const totalIncome = computeSum(BudgetType.Income);
  const elemsPercentage = getExpensesElem().querySelectorAll(
    `.${CLASS_PERCENTAGE}`
  );
